...
class CategoryFragment : BaseFragment<FragmentCategoryBinding>() {
    ...
    override fun initObservation() {
        ApplicationData.generalVideoInfoData.observe(viewLifecycleOwner) { data ->
            for (category in data.categories.entrySet()) {
                binding.categories.addView(getOptionView(category.value.asString, category.key))
            }
            ...
            for (region in data.regions.entrySet()) {
                binding.regions.addView(getOptionView(region.value.asString, region.key))
            }
            ...
            for (year in data.yearCategories.entrySet()) {
                binding.yearCategories.addView(getOptionView(year.value.asString, year.key))
            }
            binding.yearCategories.getChildAt(0).isSelected = true
            onOptionsChanged()
            homeViewModel.hotCategory.observe(viewLifecycleOwner) { category ->
    ...
    private fun onOptionsChanged() {
        ...
        pagedVideoJob = viewLifecycleOwner.lifecycleScope.launch {
            homeViewModel.getPagedVideoFlow(category, genre, region, yearCategory)
